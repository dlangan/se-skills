...
This skill produces generic, reusable strategic templates and does not read `~/.claude/se-config.json` directly — there is no account-owner, roster, or Slack/canvas identity to externalize. When a deliverable produced with this skill is written to an account workspace or Slack canvas, resolve `account_root` / `slack_team_id` / `todo_canvas_id` from config in the calling skill rather than hardcoding a path here.
...
The DSE skill should consult the Agentforce Brain for any architecture, strategy, or positioning question:
- `~/agentforce-brain/` — Full Agentforce knowledge base (32 digests). Start with INDEX.md for navigation.
- `~/agentforce-brain/digests/architecture/superagent-architecture-diagrams.md` — Agent Fabric vs Slackbot vs Agentforce decision framework
- `~/agentforce-brain/digests/architecture/multi-agent-fcd-roadmap.md` — SOMA/MOMA roadmap, customer architectures, AF vs Agent Fabric
- `~/agentforce-brain/digests/competitive/positioning-and-competition.md` — Full competitive landscape, 5 Power Pillars, objection handling
- `~/agentforce-brain/digests/architecture/value-engineering-roadmap.md` — Flex Credits ROI, industry blueprints, Systems of Action narrative
